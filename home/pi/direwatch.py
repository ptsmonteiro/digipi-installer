...
# ---------------------------------------------------------------------------
# Optional hardware imports.  On a Raspberry Pi (or other Blinka-supported
# board) these succeed and drive a real SPI TFT + GPIO LEDs.  On a generic
# PC / VM / non-Pi arm64 box they are absent, so we fall back to headless
# stubs and keep all the log parsing + PIL rendering intact.  Use -s/--savefile
# to dump the rendered frames to a PNG for inspection.
# ---------------------------------------------------------------------------
HEADLESS = False
try:
    import board
...
except (ImportError, NotImplementedError, RuntimeError, ValueError, AttributeError) as e:
    HEADLESS = True
    board = digitalio = st7789 = ili9341 = None
    print("Display libraries unavailable (%s) -- running headless." % e)

# pyinotify is Linux-only and currently unused (log is tailed via `tail -F`),
# so don't let a missing module stop the script.
try:
    import pyinotify
except ImportError:
    pyinotify = None

# libgpiod 2.x bindings + the Direction/Value enums used below.  When gpiod or
# a real gpiochip is missing we substitute no-op stand-ins (see further down).
try:
    import gpiod
...
# A line we can call .set_value()/.release() on even with no real GPIO.
class _DummyLine:
...
# Stands in for a real SPI display: .image() simply discards the frame.
# (--savefile still works because the loops save the PIL image separately.)
class _DummyDisplay:
    rotation = 0
    width = 240
    height = 240
...
if HEADLESS or displaytype == 'none':
   ## No physical display: render to a PIL image only (use -s to save PNGs)
   displaytype = 'none'
   disp = _DummyDisplay()
elif displaytype == 'ili9341':
...
if gpiod is not None and os.path.exists(chipdev):
   try:
      #setup Blue LED
      LINE = 5
      blue_line = gpiod.request_lines( chipdev, consumer="blue",
          config={
      ...
      #setup Red LED
      LINE = 26
      red_line = gpiod.request_lines( chipdev, consumer="blue",
          config={
              LINE: gpiod.LineSettings(
                  direction=Direction.OUTPUT, output_value=Value.INACTIVE
              )
      ...
      print("GPIO unavailable (%s) -- LEDs disabled." % e)
      blue_line = _DummyLine()
      red_line = _DummyLine()
else:
   blue_line = _DummyLine()
   red_line = _DummyLine()
...
def bluetooth_connection_poll_thread():  #FIXME convert to libgpiod, not gpiozero
    ...
    while True:
        cmd = "hcitool con 2>/dev/null | wc -l"
        try:
            connection_count = subprocess.check_output(cmd, shell=True).decode("utf-8")
        except Exception:
            # hcitool absent (no BlueZ) -- skip BT status reporting
            time.sleep(10)
            continue
